function [trained_ensemble,results] = ensemble_training_AllBlend(Xc,Xs,settings)
% this program is adopted from original ensemble classifier V2.0 proposed by kodovsky et al. it can
% training mixed samples with different steganography and different
% embedding rate (payload rate). The different between this version and
% original one is that this new version do not require the number of input stego sample and cover sample be the same.
% But it also requires each matrix of stego sample feature be the same size
% of matrix of cover sample feature. And cover samples, along with its
% corresponding stego samples should be put in these matrices orderly.

% we modified some problems of ensemble calssifier of original version, and the training
% process of fld base learners, and the OOB estimation.

% One must keep it in mind that the input of this function Xc and Xs are
% cells, but not matrices as in the original version.
...
% -------------------------------------------------------------------------
% INPUT: 
%  Xc - cover features£¬ a cell contains a matrix array of NxM£¬ where N is
%  number of samples, M is the dimensionality of feature. 
%  Xs - corresponding stego features,a cell
%  contains multiple matrices array of NxM, where N is
%  number of samples, M is the dimensionality of feature. Each matrix array
%  in  Xs  stores features from a kind of steganography with  one embedding rate (payload rate) 
%  settings. 
% Please note that all matrix array in Xc and Xs needs to be synchronized!
% In another word, the same row in matrixs of Xc and Xs should be features
% coming from the cover image and its corresponding stego images.
...
if mod(size(Xs,1),size(Xc,1))~=0
    error('Ensemble error: number of stego files must be a multiple of covers.');
end
...
OOB.SUB_Xc = floor(size(Xc,1)*rand(settings.randstream.bootstrap,size(Xc,1),1))+1;

OOB.SUB_Xs = [];
for stego_batch_count=0:(floor(size(Xs,1)/size(Xc,1))-1);     
OOB.SUB_Xs = [OOB.SUB_Xs;OOB.SUB_Xc+size(Xc,1)*stego_batch_count];
end



OOB.ID_Xc  = setdiff(1:size(Xc,1),OOB.SUB_Xc);
OOB.ID_Xs  = setdiff(1:size(Xs,1),OOB.SUB_Xs);
if ~isfield(OOB,'Xc')
...
num_Cover=double(length(P1));
num_Stego=double(length(P2));
...
for idTr=1:length(P)-1
    ...
    %Eact(idTr) = FA+MD;
    Eact(idTr) = FA/num_Cover+MD/num_Stego;
    %Eact2(idTr) = FA2+MD2;   
    Eact2(idTr) = FA2/num_Cover+MD2/num_Stego;
    if Eact(idTr)<Emin
        Emin = Eact(idTr);
        iopt = idTr;
        sgn=1;
...
OOB.Xc.proj = Xc(OOB.ID_Xc,base_learner.subspace)*base_learner.w-base_learner.b;
OOB.Xs.proj = Xs(OOB.ID_Xs,base_learner.subspace)*base_learner.w-base_learner.b;
OOB.Xc.num(OOB.ID_Xc) = OOB.Xc.num(OOB.ID_Xc) + 1;
OOB.Xc.fusion_majority_vote(OOB.ID_Xc) = OOB.Xc.fusion_majority_vote(OOB.ID_Xc)+sign(OOB.Xc.proj);
OOB.Xs.num(OOB.ID_Xs) = OOB.Xs.num(OOB.ID_Xs) + 1;
OOB.Xs.fusion_majority_vote(OOB.ID_Xs) = OOB.Xs.fusion_majority_vote(OOB.ID_Xs)+sign(OOB.Xs.proj);
% update errors
...
OOB.error = (sum(TMP_c>0)/(length(TMP_c))+(sum(TMP_s<0))/(length(TMP_s)))/2;
...
% FLD TRAINING
Xm = Xc(OOB.SUB_Xc,base_learner.subspace);
Xp = Xs(OOB.SUB_Xs,base_learner.subspace);
